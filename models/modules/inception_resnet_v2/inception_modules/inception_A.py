...
class InceptionA(nn.Module):
    def __init__(self, scale, in_channels=384):
        super().__init__()
        self.scale = scale
        # input 35x35x384, output 35x35x32
        self.branch1_1 = nn.Sequential(
            ConvBlock(in_channels=in_channels, out_channels=32, kernel_size=1, stride=1, padding=0),  # output: 35x35x32
        )
        ...
        self.summation = nn.Conv2d(in_channels=128, out_channels=in_channels, kernel_size=1, stride=1, padding=0) #inception preserve the channel dim
...
if __name__ == "__main__":
    in_channels = 384
    inception_a = InceptionA(scale=0.1, in_channels=in_channels)
    x = torch.randn(1, in_channels, 35, 35)  # Batch size of 1, 384 channels, 35x35 feature map
